...
class TaskItem extends Model
{
    ...
    public static function filterMyItems($params)
    {
        ...
        if ($user->employee->department->description == 'Atendimento') {
            ...
            if (User::logged()->employee->id == 57 || User::logged()->employee->id == 55 ||  User::logged()->employee->id == 11 || User::logged()->employee->id == 52 ||  User::logged()->employee->department->description == 'Orçamento') {
                $tasks->whereHas('task', function ($query) use ($user) {
                    $query->where('responsible_id', '=', 11)->orWhere('responsible_id', '=', 57)->orWhere('responsible_id', '=', 55)->orWhere('responsible_id', '=', 52);
                });
            } else {
                $tasks->whereHas('task', function ($query) use ($user) {
