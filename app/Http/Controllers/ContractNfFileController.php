...
class ContractNfFileController extends Controller
{
    public static function save(Request $request)
    {
        ...
        try {
            $project_file = ContractNfFile::insert($data);
    ...
    public static function saveMultiple(Request $request)
    {
        ...
        try {
            $projectFiles = ContractNfFile::insertAll($data);
    ...
    public static function edit(Request $request)
    {
        ...
        try {
            $project_file = ContractNfFile::edit($data);
    ...
    public static function remove(int $id)
    {
        ...
        try {
            $project_file = ContractNfFile::remove($id);
    ...
    {
        try {
            $fileFound = ContractNfFile::downloadFile($id);
            $status = true;
    ...
    {
        try {
            $fileFound = ContractNfFile::downloadAllFiles($taskId);
            $status = true;
            return Response::make(file_get_contents($fileFound), 200, ['Content-Type' => mime_content_type($fileFound)]);
        } catch (Exception $e) {
            $message = 'Um erro ocorreu ao abrir o arquivo: ' . $e->getMessage();
