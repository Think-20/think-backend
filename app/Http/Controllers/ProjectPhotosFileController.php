...
class ProjectPhotosFileController extends Controller
{
    public static function save(Request $request)
    {
        ...
        try {
            $project_file = ProjectPhotos::insert($data);
    ...
    public static function saveMultiple(Request $request)
    {
        ...
        try {
            $projectFiles = ProjectPhotos::insertAll($data);
    ...
    public static function edit(Request $request)
    {
        ...
        try {
            $project_file = ProjectPhotos::edit($data);
    ...
    public static function remove(int $id)
    {
        ...
        try {
            $project_file = ProjectPhotos::remove($id);
    ...
    {
        try {
            $fileFound = ProjectPhotos::downloadFile($id);
            $status = true;
    ...
    {
        try {
            $fileFound = ProjectPhotos::downloadAllFiles($taskId);
            $status = true;
            return Response::make(file_get_contents($fileFound), 200, ['Content-Type' => mime_content_type($fileFound)]);
        } catch (Exception $e) {
            $message = 'Um erro ocorreu ao abrir o arquivo: ' . $e->getMessage();
