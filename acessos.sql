...
OR (f.id IN(59,61,63,64,65,66,67,69,71,72,73,74,75,77,78,79));
...
OR f.id = 39 OR (f.id >= 52 AND f.id <= 58) OR (f.id IN(60,62,68,70,71,72,73,74,80,81,82));
...
left join user u on u.employee_id = e.id
where (f.id >= 48 AND f.id <= 51) OR (f.id IN(60,62,68,81));
